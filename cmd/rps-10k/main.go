...
func main() {
    ...
    fmt.Printf("--- Contextual SSN (10k Stress Test) ---\n")
    ...
    logPath := "game_analysis_10k.csv"
    for _, dir := range logDirs {
        if _, err := os.Stat(dir); err == nil {
            logPath = fmt.Sprintf("%s/game_analysis_10k.csv", dir)
            break
        }
    }
    ...
    for t := 1; t <= 10000; t++ {
        ...
        if t % 500 == 0 { // Log every 500 steps to keep output clean
            fmt.Printf("Step %5d | Context: %-8s | WinRate: %.1f%%\n", 
